...
public class ClientCore implements ICSMessageEventReceiver {
	...
	public void ChangeArticleData(Article article, IClientGUIListener customer) {
		opID++;
		ChangeArticleMessageRequest caMsg = new ChangeArticleMessageRequest(
				article);
	...
	public void DeleteArticle(int articleID, IClientGUIListener customer) {
		opID++;
		DeleteArticleMessageRequest delMsgReq = new DeleteArticleMessageRequest(
				articleID);
		ccm.addMessage(opID, delMsgReq, this);
		map.put(new Integer(opID), customer);
	}

	public void BuyArticle(Article article, float amount,
	...
	public void ChangeUserData(User user, IClientGUIListener customer) {
		opID++;
		ChangeUserDataMessageRequest numrMsg = new ChangeUserDataMessageRequest(
				user);
	...
	public void AnswerReceived(int opID, IVBMessage answer) {
		...
		case LoginMessageAnswer:
			if (((LoginMessageAnswerPayload) answer.getPayload())
					.isLoginSucces()) {
				this.user = ((LoginMessageAnswerPayload) answer.getPayload())
						.getUserData();
				customer.positiveAnswerReceived(answer.getPayload());
			} else {
				customer.failureAnswerReceived("Login failed: "
						+ ((LoginMessageAnswerPayload) answer.getPayload())
								.getFailureReason());
			}
			break;
	...
	void GetUpdatedUserData(IClientGUIListener customer) {
		opID++;
		GetUpdatedUserDataRequest msg = new GetUpdatedUserDataRequest(
				this.user.getId());
		ccm.addMessage(opID, msg, this);
		map.put(opID, customer);
	}
