...
public class Client {
	...
	ccm.setPort(Integer.parseInt(args[0]));
	while (true) {
	    clearConsole();
	...
	NewUserConfirmationCodeRequest msg = new NewUserConfirmationCodeRequest(
		mail, code);
	NewUserConfirmationCodeAnswer ans;
	try {
	    ans = ((NewUserConfirmationCodeAnswer) ccm.SendMessage(msg));
	    GenericAnswerPayload payload = ((GenericAnswerPayload) ans
		    .getPayload());
	    if (payload.isOperationSuccess()) {
		System.out.println("User confirmed, you can login now.");
	...
	while (true) {
	    ...
	    usr.setSurname(str);
	    System.out
		    .println("Insert your e-Mail (This will also be your username):");
	    str = System.console().readLine();
	    usr.seteMail(str);
		...
		if (p1.equals(p2)) {
		    ...
		    System.out
			    .println("Password does not match, please repeat.");
	    }
	    System.out.println("");
	    ...
	    switch (i) {
	    case 1:
		usr.setState("UNCONFIRMED");
		NewUserMessageRequest numrMsg = new NewUserMessageRequest(usr,
			pwd);
		try {
	...
	while (true) {
	    ...
	    case 1:
		messages.ChangeUserDataMessageRequest numrMsg = new messages.ChangeUserDataMessageRequest(
			user);
		try {
		    ccm.SendMessage(numrMsg);
		} catch (ClientConnectionException e) {
	...
	while (true) {
	    ...
	    System.out
		    .println("Do you want to insert the (1)Gross price or the (2)Net price?");
	    str = System.console().readLine();
	    ...
	    switch (Integer.parseInt(str)) {
	    case 1:
		messages.AddArticleMessageRequest aaMsg = new messages.AddArticleMessageRequest(
			art);
		try {
	...
	try {
	    messages.ArticleListMessageAnswer alMsg = (messages.ArticleListMessageAnswer) ccm
		    .SendMessage(new messages.ArticleListMessageRequest(user
			    .getId(), true));
	    ArticleListAnswerPayload payload = ((ArticleListAnswerPayload) alMsg
		    .getPayload());
	    artList = payload.getList();
	} catch (ClientConnectionException e) {
	    System.out.println(e.getMessage());
	    return 0;
	}
	while (true) {
	    ...
	    case 3:
		System.out
			.println("Insert the number of the selected article:");
		str = System.console().readLine();
	...
	while (true) {
		...
		art.setWeight(Float.parseFloat(str));
		System.out
			.println("Please insert the VAT for the articles as %:");
		str = System.console().readLine();
		art.setVat(Float.parseFloat(str));
		System.out
			.println("Do you want to insert the (1)Gross price or the (2)Net price?");
		...
		switch (Integer.parseInt(str)) {
		case 1:
		    messages.ChangeArticleMessageRequest caMsg = new messages.ChangeArticleMessageRequest(
			    art);
		    article = art;
		    try {
	...
	for (int i = 0; i < elemNr && (i + offset) < artV.size(); i++) {
	    System.out.println(i + offset + ") "
		    + artV.get(offset + i).getShortDescription() + "  Amount: "
		    + artV.get(offset + i).getAmount() + "  Price: "
		    + artV.get(offset + i).getNetPrice() + " Euro");
	}
	System.out
		.println("=========================================================================");
    }
	...
	while (true) {
		...
		case 1:
		    ...
		    if (amt > article.getAmount() || amt <= 0) {
			System.out.println("Insert the correct amount.");
			break;
		    }
		    try {
			ccm.SendMessage(new messages.BuyArticleMessageRequest(
				user.getId(), article, amt));
			return;
		    } catch (ClientConnectionException e) {
			System.out.println(e.getMessage());
			System.out.println("(1) Retry or (2) Back");
