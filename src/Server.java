...
public class Server implements Runnable {
	...
	User res = new User();
	res.fillFromResultSet(rs);
	query = "SELECT * FROM SEPA WHERE idNutzer=" + userId + ";";
	rs = con.ExecuteQuery(query);
	if (rs.next()) {
	    SEPA bnk = new SEPA();
	    bnk.fillFromResultSet(rs);
	    ...
	    query = "SELECT * FROM Bankverbindung WHERE idNutzer=" + userId
		    + ";";
	    rs = con.ExecuteQuery(query);
	    if (rs.next()) {
		BankConnection bnk = new BankConnection();
	...
	try {
	    ...
	    String cmd = user.getUpdateStatement();
	    System.out.println(cmd);
	    con.AddCommand(cmd);
	    if (user.getBankData().getType() == "SEPA") {
		SEPA bnk = ((SEPA) user.getBankData());
		bnk.setUserId(user.getId());
		con.AddCommand(bnk.getUpdateStatement());
	    } else {
