...
public class Article implements IDBObject {
	...
	if (shortDescription.length()>140)
	    throw new SQLException("Bezeichnung can be miximal 140 characters long.");
	String sqlStatement="UPDATE Artikel SET ";
	sqlStatement+="Bezeichnung="+SQLDataUtilities.getSQLDataFormat(shortDescription)+", ";
	sqlStatement+="Beschreibung="+SQLDataUtilities.getSQLDataFormat(longDescription)+", ";
	...
	sqlStatement+="Preis_Brutto="+SQLDataUtilities.getSQLDataFormat(grossPrice)+", ";
	sqlStatement+="Preis_Netto="+SQLDataUtilities.getSQLDataFormat(netPrice)+", ";
	sqlStatement+="AblaufDatum="+SQLDataUtilities.getSQLDataFormat(expDate)+", ";
	sqlStatement+="idNutzer="+SQLDataUtilities.getSQLDataFormat(idUser)+" ";
	sqlStatement+="WHERE idArtikel="+SQLDataUtilities.getSQLDataFormat(id)+";";
	return sqlStatement;
    }
	...
	if (shortDescription.length()>140)
	    throw new SQLException("Bezeichnung can be miximal 140 characters long.");
	String sqlStatement="INSERT INTO Artikel (Bezeichnung, Beschreibung, Gewicht, Anzahl, MwSt, Preis_Brutto, Preis_Netto, AblaufDatum, idNutzer) VALUES(";
	sqlStatement+=""+SQLDataUtilities.getSQLDataFormat(shortDescription)+", ";
